...
public class MouseLook : MonoBehaviour {
    ...
    void LookAround() {
        ...
        look_Angles.x += current_Mouse_Look.x * sensivity * (invert ? 1f : -1f); // if invert ? (true) then 1f : (else) -1f 
        look_Angles.y += current_Mouse_Look.y * sensivity;

        look_Angles.x = Mathf.Clamp(look_Angles.x, default_Look_Limits.x, default_Look_Limits.y); // this will put a limit to the x and y axis of look_Angles(same as if statement)
